...
@section('title', 'Assign Cutting Workers - Step 2 of 3')
...
@section('content')
<div class="space-y-4" x-data="cuttingAssignmentWizard()">

    {{-- Page Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
            <div class="flex items-center gap-2">
                <span class="px-2.5 py-0.5 rounded-md bg-indigo-100 text-indigo-700 text-[11px] font-bold uppercase tracking-wider">Step 2 of 3</span>
                <h2 class="text-xl font-bold text-slate-800">Assign Cutting Workers (کٹنگ کاریگر)</h2>
            </div>
            <p class="text-xs text-slate-500 mt-0.5">Choose cutting tailors for {{ count($orders) }} selected order(s), then proceed to Stitching assignment.</p>
        </div>
    ...
    @if (count($orders) > 1)
    <div class="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white px-4 py-3 rounded-2xl shadow-md border border-indigo-900/60 overflow-x-auto">
        <div class="flex items-center justify-between gap-4 min-w-[680px]">
            {{-- Left: Title & Subtitle --}}
            <div class="flex items-center gap-2.5 shrink-0">
                <span class="inline-flex items-center justify-center w-7 h-7 rounded-xl bg-amber-400/20 text-amber-400 font-black text-sm shrink-0">⚡</span>
                <div>
                    <h4 class="text-xs font-bold text-white uppercase tracking-wider whitespace-nowrap">
                        Quick Apply: Set Same Cutting Worker for All {{ count($orders) }} Orders
                    </h4>
            ...
            <div class="flex items-center gap-2 shrink-0 ml-auto">
                <select x-model="bulkCuttingId" class="w-64 sm:w-72 py-1.5 px-3 bg-slate-800 border border-slate-700 text-white text-xs font-medium rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-400 shadow-inner">
                    <option value="" class="bg-white text-slate-800">-- Select Cutting Tailor for All --</option>
                ...
                <button type="button" @click="applyBulkCutting()" class="px-4 py-1.5 bg-indigo-500 hover:bg-indigo-600 text-white font-bold text-xs rounded-xl shadow-md transition transform active:scale-95 whitespace-nowrap shrink-0 cursor-pointer">
                    Apply to All
                </button>
            </div>
        </div>
    ...
    {{-- Form for Step 2 -> Step 3 Submission --}}
    <form method="POST" action="{{ route('assign-orders.stitching') }}" class="space-y-4">
        @csrf
            ...
            @foreach ($orders as $index => $order)
                ...
                <div class="bg-white rounded-2xl border border-slate-200/90 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden">
                    ...
                    <div class="p-3.5 space-y-3 flex-1 flex flex-col justify-between">
                        ...
                        {{-- Worker Assignments Section: Cutting Only --}}
                        <div class="pt-2 border-t border-slate-100">
                            <div>
                                <label class="block text-xs font-bold text-slate-700 mb-1 flex items-center gap-1.5">
                                    <span class="text-sm">✂️</span>
                                    <span>Assign Cutting Worker</span>
                                </label>
                                <select name="assignments[{{ $index }}][cutting_employee_id]"
                                        x-model="cuttingAssignments[{{ $order->id }}]"
                                        class="block w-full py-2 px-3 bg-slate-50 hover:bg-white focus:bg-white border border-slate-300 hover:border-slate-400 rounded-xl text-xs font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 shadow-xs transition">
                                    <option value="">-- Choose Cutting Tailor --</option>
                                    @foreach ($cuttingEmployees as $emp)
                                        <option value="{{ $emp->id }}">
        ...
        <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3 mt-6">
            <a href="{{ route('assign-orders.index') }}" class="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-slate-300 text-slate-700 font-semibold text-xs rounded-xl hover:bg-slate-50 transition">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18"/></svg>
                Back to Selection
            </a>

            <div class="flex items-center gap-3 flex-wrap justify-end">
                {{-- Option 1: Direct Save Cutting Only --}}
                <button type="submit" formaction="{{ route('assign-orders.save') }}"
                        class="inline-flex items-center gap-1.5 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-sm hover:shadow transition transform active:scale-95 cursor-pointer">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                    <span>Save Cutting Assignment</span>
                </button>

                {{-- Option 2: Proceed to Stitching Step 3 --}}
                <button type="submit" class="group inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold text-xs rounded-xl shadow-md shadow-indigo-500/25 transition transform hover:scale-[1.01] active:scale-95 cursor-pointer">
                    <span>Next: Assign Stitching (Step 3)</span>
...
@push('scripts')
<script>
    function cuttingAssignmentWizard() {
        const defaults = {};
        @foreach ($orders as $order)
            defaults[{{ $order->id }}] = '{{ $existingCutting[$order->id] ?? $order->cutting_employee_id }}';
        @endforeach

        return {
            bulkCuttingId: '',
            activeModalOrder: null,
            cuttingAssignments: defaults,
            init() {
                this.$watch('bulkCuttingId', () => this.applyBulkCutting());
            },
            applyBulkCutting() {
                if (!this.bulkCuttingId) return;
                Object.keys(this.cuttingAssignments).forEach(id => {
                    this.cuttingAssignments[id] = String(this.bulkCuttingId);
                });
            }
        }
    }
