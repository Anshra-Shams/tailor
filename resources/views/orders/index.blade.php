            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50">
                        ...
                        <th class="text-left px-5 py-3 font-semibold text-slate-600">Customer</th>
                        <th class="text-left px-5 py-3 font-semibold text-slate-600">Member</th>
                        <th class="text-left px-5 py-3 font-semibold text-slate-600">Service</th>
                        <th class="text-left px-5 py-3 font-semibold text-slate-600">Price</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($orders as $order)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-5 py-3 font-mono text-slate-500 text-xs">#{{ $order->id }}</td>
                            <td class="px-5 py-3 font-medium text-slate-700">{{ $order->customer->name ?? '—' }}</td>
                            <td class="px-5 py-3 text-slate-500">{{ $order->member->name ?? '—' }}</td>
                            <td class="px-5 py-3 text-slate-500">{{ $order->service->name ?? '—' }}</td>
                            <td class="px-5 py-3 font-medium text-slate-700">Rs {{ number_format($order->price) }}</td>
                            <td class="px-5 py-3">
                                @php
                                    $statusColors = [
                            ...
                            <td class="px-5 py-3 text-slate-500 text-xs">{{ $order->order_date?->format('M d, Y') ?? '—' }}</td>
