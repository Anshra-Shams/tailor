            ...
            @foreach($customers as $customer)
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="flex items-center gap-2 pt-3 border-t border-slate-100">
                        <a href="{{ route('customers.show', $customer) }}" class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-200 rounded-lg hover:bg-emerald-100 transition-all">
                        ...
                        <form method="POST" action="{{ route('customers.destroy', $customer) }}" onsubmit="return confirm('Delete this customer and all their members?')" class="flex-1">
                            @csrf
                            @method('DELETE')
